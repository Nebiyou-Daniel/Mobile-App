...
class TraineeBottomNavigation extends StatelessWidget {
  int selectedIndex;
  // a list of widgets to be displayed in the bottom navigation bar

  TraineeBottomNavigation({required this.selectedIndex, super.key});

  void _onItemTapped(int index, context) {
    selectedIndex = index;
    if (selectedIndex == 0) {
      GoRouter.of(context).go('/trainee/homePage');
    } else if (selectedIndex == 1) {
      GoRouter.of(context).go('/trainee/profile');
            ...
            child: Column(
              children: [
                ListTile(
                  ...
                  onTap: () {
                    context.push("/trainee/chooseTrainer");
                  },
                ),
                ListTile(
